...
const Doctor = ({ipfs, healthTrust, account}) => {
  ...
  const getDoctorData = async () => {
    var doctor = await healthTrust.methods.doctorInfo(account).call();
    setDoctor(doctor);
  }
  const getPatientAccessList = async () => {
    var pat = await healthTrust.methods.getDoctorPatientList(account).call();
    let pt = []
    for(let i=0; i<pat.length; i++){
      let patient = await healthTrust.methods.patientInfo(pat[i]).call();
  ...
  const getTransactionsList = async () => {
    var transactionsIdList = await healthTrust.methods.getDoctorTransactions(account).call();
    let tr = [];
    for(let i=transactionsIdList.length-1; i>=0; i--){
        ...
        let sender = await healthTrust.methods.patientInfo(transaction.sender).call();
  ...
  const submitDiagnosis = async (e) => {
    ...
    ipfs.add(record).then((result, error) => {
      if(error){
        ...
        healthTrust.methods.updatePatientRecordAndAddTransaction(patient.account, result.path, charges).send({from: account}).on('transactionHash', (hash) => {

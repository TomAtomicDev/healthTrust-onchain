...
const Register = ({healthTrust, ipfs, connectWallet, token, account, setToken, setAccount}) => {
        ...
        if(account!=="" && designation==="1"){
            ...
            ipfs.add(record).then((result, error) => {
                if(error){
                    ...
                    healthTrust.methods.register(name, age, parseInt(designation), email, result.path).send({from: account}).on('transactionHash', async (hash) => {
                        window.location.href = '/login'
                    })
                }
            })
        }else if(account!==""){
            healthTrust.methods.register(name, 0, parseInt(designation), email, "").send({from: account}).on('transactionHash', async (hash) => {
                window.location.href = '/login'
            })
        }
    }
